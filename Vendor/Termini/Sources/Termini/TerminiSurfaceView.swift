...
public final class SurfaceContainerView: NSView {
    ...
    /// Writes a selection (or explicit copy) out to the system pasteboard.
    /// `confirmed` distinguishes an OSC 52 write awaiting user confirmation
    /// from a direct copy (mouse selection, `copy_to_clipboard` keybind);
    /// Termini has no confirmation UI, so an OSC 52 write is left a no-op
    /// here rather than silently trusting remote-supplied clipboard content.
    func writeClipboard(
        location: ghostty_clipboard_e,
        ...
        guard !confirmed, count > 0, let pasteboard = pasteboard(for: location) else { return }

        var plainText: String?
        var firstText: String?
        for index in 0..<count {
            guard let dataPtr = content[index].data else { continue }
            let string = String(cString: dataPtr)
            if firstText == nil { firstText = string }
            if let mimePtr = content[index].mime, String(cString: mimePtr) == "text/plain" {
                plainText = string
                break
            }
        }
        guard let text = plainText ?? firstText else { return }
