...
5. The bootstrap runs as the command of that session's first window: seed
   ...
   (`https://github.int.exe.xyz/<owner>/<repo>.git`) — so you watch it happen in
   the first tab, and it doesn't run again when you reattach.
...
- **Start command** — what runs in the first tmux window after the bootstrap,
  and only when the tmux session is first created (e.g. `claude`). Reconnecting
  attaches instead of starting a second copy. When it exits the window stays
  open at a shell. Empty means just a shell. tmux itself is not configurable and
  is required — the app is a tmux client, so the remote command installs tmux
  first if the VM lacks it.
...
```
┌───────────┬──────────────────────────┬─────────────────┐
│ Session   │ tmux windows as tabs      │  Worktree Diff  │
│ tabs      ├──────────────────────────┤  (git diff of   │
│ (⌘T/⌘W)   │   Active terminal         │   the terminal's│
│           │   (SwiftTerm)             │   cwd)          │
└───────────┴──────────────────────────┴─────────────────┘
```

SwiftUI provides the window chrome and both sidebars; the terminals are
SwiftTerm views. A local shell uses `LocalProcessTerminalView`, which spawns a
shell over a PTY. A VM session uses plain `TerminalView`s fed by tmux instead.
...
- each pane's bytes are fed to that pane's own `TerminalView` — one tab each;
...
directory via OSC 7. SwiftTerm surfaces that through its
...
`git diff HEAD` for that directory whenever it changes. A VM session's panes are
inside tmux, which consumes OSC 7 itself, so the path comes from tmux's
