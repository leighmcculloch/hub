...
/// `https://api.sprites.dev/v1/gateway/openrouter/<connection_id>/…` — an
/// OpenAI Chat Completions API, callable only from inside a sprite, with a
/// per-org `connection_id` discovered at runtime. That's not something the
/// harnesses can be pointed at directly: Claude Code speaks the Anthropic
/// Messages API, and the URL isn't known at VM-create time. So a small Python
/// proxy runs on the sprite, listens on localhost, discovers the connector's
/// `gateway_base_url` from the gateway list endpoint, translates Anthropic
/// Messages ↔ OpenAI Chat Completions for Claude Code, and passes OpenAI
/// requests through for Codex and pi.
///
/// Kept out of `Bootstrap`, which only decides *when* it is installed (the
/// `HarnessWiring.setup` fragment for sprites.dev). The script is a raw literal
/// so the Python reads as Python.
enum SpriteLLMProxy {
    ...
    static let scriptName = ".sprite-llm-proxy.py"
    static let logName = ".sprite-llm-proxy.log"

    /// Emitted in the bootstrap's harness `setup` fragment: writes the script
    /// and starts it in the background, then waits for it to listen so the
    /// harnesses find it when they start. Re-runnable on reconnect: an existing
    ...
    static var install: String {
        ...
        pkill -f "$HOME/\(scriptName)" 2>/dev/null || true
        nohup python3 "$HOME/\(scriptName)" >> "$HOME/\(logName)" 2>&1 &
        for _i in $(seq 1 30); do
          if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 1 \(baseURL)/health >/dev/null 2>&1; then break; fi
          sleep 0.2
        done
    ...
    ///
    /// TODO(prove): exercise against a live sprite with a real OpenRouter
    /// connector. The translation is written to the public Anthropic Messages
    /// and OpenAI Chat Completions shapes but has only been read, not run.
    static var script: String {
        #"""
        #!/usr/bin/env python3
        # An LLM proxy for sprites.dev: exposes the Anthropic Messages API and the
        # OpenAI Chat Completions API on localhost, forwarding through the
        # OpenRouter connector gateway (discovered via the gateway list endpoint).
        # Anthropic Messages requests are translated to OpenAI Chat Completions;
        # OpenAI requests pass through. Streaming is translated too.
        #
        # Authentication is implicit: the gateway identifies the calling sprite
        # from Fly.io's request signature, so no token is needed inside the VM.
        ...
        def discover_gateway():
            # The list endpoint returns the connectors this sprite can reach;
            # the OpenRouter one's gateway_base_url is what we forward to.
        ...
        # MARK: - Anthropic Messages -> OpenAI Chat Completions

        def anthropic_content_to_openai(content):
            # Anthropic content is a string or a list of typed blocks; OpenAI
            # wants a string or a list of {type, ...} parts.
        ...
        def anthropic_to_openai(body):
            ...
            for msg in body.get("messages", []):
                ...
                if isinstance(content, list):
                    # Split out tool_result blocks: OpenAI carries each as a
                    # separate tool message; text/image become one user message.
        ...
        # MARK: - Streaming translation (OpenAI SSE -> Anthropic SSE)
        ...
        def anthropic_stream_events(openai_iter, model, input_tokens):
            ...
            # OpenAI streams text and tool_call fragments interleaved; Anthropic
            # wants one content block per block index. Track open blocks.
            blocks = {}  # index -> {"type": ..., "id": ..., "name": ...}
            next_index = 0
            text_index = None
            output_tokens = 0
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _messages(self, body):
                ...
                try:
                    ...
                    # Surface the gateway's error as an Anthropic-shaped error so
                    # Claude Code can show it rather than a raw HTTP code.
                    err = json.dumps({
                        "type": "error",
        ...
        def main():
            try:
                ...
                # Don't fatal-exit: the gateway may not be provisioned yet, and a
                # later request will retry discovery. Log and continue.
